...
show_help_overlay = True
# Delay actual drawing until the figure canvas is ready
fig.canvas.mpl_connect('draw_event', lambda evt: safe_update_help_overlay())
...
def init_channels(n):
    ...
    Called once upon first received sample.
    """
    global num_channels, channel_names, data_buffers, time_lines, time_legend
    num_channels = n
...
def safe_update_help_overlay():
    """Ensure help overlay updates run in the main GUI thread."""
    if threading.current_thread() is threading.main_thread():
        update_help_overlay()
    else:
        fig.canvas.manager.canvas.after(0, update_help_overlay)
